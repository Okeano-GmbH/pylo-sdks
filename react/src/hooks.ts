...
import { usePyloTransport } from "./provider.js";
...
export type InfiniteListOptions<S, E extends EntityNameWith<S, "list">, Sel extends EntitySelect<S, E>> = {
  perPage?: number;
  select: Sel;
  filter?: FilterInput;
} & RequestOptions;
...
export interface UploadHookOptions<S> {
  /**
...
export interface StartUploadOptions<S> {
  entityRelationPath?: EntityRelationPath<S>;
  isPublic?: boolean;
...
export interface UploadHookResult<S> {
  ...
  isUploading: boolean;
  /**
   * 0–100 across the current batch, weighted by bytes. React Native streams
   * from a uri and reports no file size, so those batches weight files equally.
   */
  progress: number;
  error: Error | null;
  ...
  >(
    ...
    const result = useQuery({
      ...
      queryFn: async () => {
        ...
        const data = (await transport(query, variables, merged)) as Record<
          string,
        ...
        try {
          ...
          const results = await Promise.all(
            list.map(async (_source, index) => {
              ...
              await uploadToUrl(uploadUrl.url, part.part, part.fileName, {
              ...
              loadedPerFile[index] = part.size ?? loadedPerFile[index]!;
          ...
          if (opts.attachTo) {
            ...
            await transport(query, variables, merged);
